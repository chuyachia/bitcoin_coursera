...
public class Main {
   ...
   public static void main(String[] args) throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
		...
		KeyPair pk_yaya  = KeyPairGenerator.getInstance("RSA").generateKeyPair();
        /*
		...
		ArrayList<Transaction.Output> tx2Outputs = tx2.getOutputs();
		for(int j = 0;j<tx2Outputs.size();j++){
			System.out.println(tx2Outputs.get(j).value);
		}
		//
		Tx tx3 = new Tx();
        ...
        tx4.addOutput(3, pk_scrooge.getPublic());
		...
		//System.out.println("txHandler.isValidTx(tx2) returns: " + txHandler.isValidTx(tx3));
        System.out.println("txHandler.handleTxs(new Transaction[]{tx2}) returns: " +
            txHandler.handleTxs(new Transaction[]{tx4,tx3,tx2}).length + " transaction(s)");
    }
